...
fn main() -> anyhow::Result<()> {
    ...
    if args.train_tokenizer {
        ...
        println!("training decoder");
        let t0 = std::time::Instant::now();
        let decoder: TokenDecoder<T> = tokenizer.to_decoder();
        let t1 = std::time::Instant::now();
        let training_duration = t1.duration_since(t0);
        println!("decoder training_duration: {:#?}", training_duration);
        ...
        if args.time_encode_decode {
            ...
            println!("decoder.decode_to_string avg duration: {:#?}", decode_avg);
        }
    }
